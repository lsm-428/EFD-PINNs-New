...
def test_phi_range_constraint():
    ...
    assert abs(opening_rate - 0.375) < 0.01, \
        f"Expected opening rate 0.375, got {opening_rate:.3f}"
...
def test_field_dimensions():
    ...
    assert 100e-6 <= Lx <= 300e-6, \
        f"Lx = {Lx*1e6:.0f}μm out of expected pixel range [100, 300]μm"
...
def test_bottom_grid_sampling():
    ...
    assert pts.shape == (n_grid * n_grid, 6), \
