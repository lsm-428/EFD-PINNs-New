...
for name, run_dir in RUNS:
    ...
    for vf, vt, ts in tests:
        pts = torch.cat(
            [
                ...
                torch.full((n_vol, 1), float(ts), device=device),
            ],
            dim=1,
        )
        with torch.no_grad():
    ...
    n_ap = 96
    xa = np.linspace(0, Lx, n_ap)
    ya = np.linspace(0, Ly, n_ap)
    ...
    n_if = 128
    xi = np.linspace(0, Lx, n_if)
    yi = np.linspace(0, Ly, n_if)
    ...
    for angle in np.linspace(0, 2 * np.pi, 9)[:8]:
        ...
        lo = np.where(phi_r > 0.1)[0]
        hi = np.where(phi_r < 0.9)[0]
