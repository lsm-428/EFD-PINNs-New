...
| 面 | 位置 | φ | 约束类型 | 说明 |
|------|------|---|---------|------|
| **顶面** | z = Lz = 20μm | φ = 0 | 软约束 | ITO 玻璃，始终接触极性液体；φ=1 → 器件失效 |
| **底面** | z = 0 | 由电润湿决定 | 湿润面主导 | Teflon 疏水层，Young-Lippmann 调制接触角 |
| **围堰壁立面 × 4** | x=0/Lx, y=0/Ly | 见下方分层 | 惩罚约束 | 4 个相同的围堰立壁 |

**围堰壁立面（4 个侧面）φ 分层**：
- **d_wall = 0，0 < z < 3.5μm**（围堰立壁面）：**φ = 1**（钉扎面，Teflon 亲油，油墨始终附着）
- **d_wall = 0，z = 3.5μm**：**φ = 0.5**（壁顶接触线，油墨-水-SU8 三相接触线，始终不动）
- **3.5μm < z < Lz**：**φ = 0**（壁顶面以上，全是水）

> 注：d_wall = min(x, Lx-x, y, Ly-y) 表示点到最近壁面的距离。
> 围堰壁立面的约束通过数据驱动惩罚实现，非硬约束。

**顶面 (z = Lz = 20μm)**：ITO 玻璃极板，始终接触极性液体。
- **φ = 0 永远成立**。任何位置出现 φ=1 意味着油墨接触顶面 → **器件永久失效**

**壁顶面（像素墙顶面，z = wall_height = 3.5μm）**：SU-8 暴露面（亲水，θ=125°）
...
**壁顶接触线（围堰立面与壁顶面顶角）**：油墨-水-SU8 三相接触线，始终不动
- **φ = 0.5**（三相接触线）
- 位置：z = 3.5μm，d_wall = 0（仅在围堰立壁面上）
...
- 壁顶接触线：z=3.5μm，d_wall=0 处 φ=0.5
- 壁顶面以上（z > 3.5μm）：全是水，φ=0
- 底板油膜呈**倒球冠**形态：
  - 底面（z=0）：φ=1（油墨面朝底面）
  - 顶面（z≈h_ink≈3μm）：φ=0.5（油面朝上，与极性液体交界面）
  - 侧面（d_wall=0, 0<z<3.5μm）：φ=1（钉扎面，Teflon 亲油）
  - 边缘（z=3.5μm, d_wall=0）：φ=0.5（壁顶接触线）
  - 中心略低（<3.5μm），近似平铺

**油膜初始状态（t=0, V=0）**：
- 油膜呈**倒球冠**形态平铺在底部
- 底面（z=0）：φ=1（油墨）
- 顶面（z≈h_ink≈3μm）：φ=0.5（油水交界面）
- 侧面（d_wall=0, 0<z<3.5μm）：φ=1（钉扎面，Teflon 亲油）
- 边缘（z=3.5μm, d_wall=0）：φ=0.5（壁顶接触线）
- 中心略低于边缘，近似平铺

**初始状态判定**：
- 判定标准：网络中 >90% 的采样点满足油水交界面 φ(x,y,z=h_ink=3μm)=0.5，即认为回到初始状态
- **t=0, V=0**：初始状态
- **t=任意, V≤V_T（3V）**：电压不足，不驱动，接近初始状态
- **t<2ms, V=任意**：响应时间不足，接近初始状态
- **升压状态（t>2ms, V>V_T）**：电润湿驱动，(x,y,z=0)平面部分区域φ=0，出现开口率
  - 开口模式：中心开口（油墨环形分布在边缘）
  - ΔV越大 → 开口率越大；ΔV越小 → 开口率越小
  - 开口半径 r_open = √(η×Lx×Ly/π)
  - 油墨堆高 h_ink_edge = Lx×Ly×h_ink / (Lx×Ly - π×r_open²)（体积守恒）
  - 开口率 < 50%：中心开口模式（油墨环形分布）
  - 开口率 > 50%：四角液滴模式（油墨堆积在角落）
- **降压状态（V_to < V_from）**：
  - 也有开口，但开口率随时间指数下降
  - η(t) = η_at_step × exp(-t_since / τ_recovery)
  - 电润湿力减弱 → 表面张力驱动油墨从边缘向中心铺展
  - **电压瞬降至0~V_T**：t+ 时刻指数恢复至初始状态
  - **电压缓降至V_T**：t+2ms 恢复至初始状态

**IC 完整条件**：

**物理定义**：t=0, V=0，油膜呈倒球冠平铺在底部，无开口。
...
|------|------|------|
| 初始 | t=0, V=0 | 未驱动 |
| 未驱动 | t=任意, V≤V_T(3V) | 电压不足，不驱动 |
| 驱动早期 | t<2ms, V=任意 | 响应时间不足 |
| 降压 | V→0~V_T（瞬降） | t+ 时刻指数恢复 |
| 降压 | V→V_T（缓降） | t+2ms 恢复 |

**什么时候不是初始状态**：

| 工况 | 条件 | 结果 |
|------|------|------|
| 升压 | t>2ms, V>V_T | 电润湿驱动，(x,y,z=0)平面部分区域φ=0，出现开口率 |
| 升压 | ΔV越大 | 开口率越大；ΔV越小，开口率越小 |
| 降压中途 | V_from > V_to > 0 | 也有开口，开口率 η(t)=η_at_step×exp(-t/τ_recovery) 指数下降 |

**三层实现**：

**Layer 1 硬约束（forward blend，不可违反）**：
- z > wall_height：blend=1，始终自由
- t<2ms 或 V<V_T：blend=0，强制 φ=phi_ic
- t>2ms, V>V_T, z≤wall_height：blend=1，自由
- phi_ic：z<h_ink→φ=1 | h_ink≤z≤wall_height→tanh过渡 | z>wall_height→φ=0.5(中性)

**Layer 2 数据驱动（MSE loss）**：
- 采样：z∈[0, wall_height], V∈[0,30V] 随机
- 目标：z<h_ink→φ=1 | z=h_ink→φ=0.5(交界面) | h_ink<z<wall_height→tanh过渡 | z>wall_height→φ=0.5
- 速度目标：u=v=w=0, p=0
- 损失：MSE_φ × (300×ic_phi_mult) + MSE_vel × 50

**Layer 3 权重退火**：
- S1: ic_phi_mult=1.0, 有效权重=300
- S2: ic_phi_mult=1→0.5, 有效权重=300→150
- S3: ic_phi_mult=0.5→0.3, 有效权重=150→90

**顶板（z = Lz = 20μm）**：ITO 玻璃，始终 φ=0
