...
class Trainer:
    ...
    def _compute_contact_angle_loss(self, data: dict[str, torch.Tensor]):
        ...
        物理：
        - 接触角 θ 是油-水-疏水层系统的接触角，从油侧测量
        - 界面法向 n = ∇φ/|∇φ| 指向 φ 增加方向（水→油）
        - cos(θ) = -n_z（从油侧测量）
        - 第二接触线处 φ=0.5（油-水界面）

        静态约束: cos(θ_local) = cos(θ_eq)  @ z=0
        ...
        # cos(θ) = -dphi_dz / grad_mag（从油侧测量，界面法向指向 φ 增加方向）
        ...
        loss_slip = torch.mean((v_slip - alpha_slip * delta_cos) ** 2) * 10.0
    ...
    def _compute_volume_conservation_loss(self, stage1_factor: float):
        ...
        v_from_all = torch.tensor([t[0] for t in tests], device=self.device).repeat(n_vol)  # (N*n_vol,)
