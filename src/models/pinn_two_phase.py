...
class TwoPhasePINN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if getattr(self, "use_hard_constraints", False):
            ...
            #    物理：油墨在 z∈[0, h_ink=3μm]，壁顶接触线 z=3.5μm
            #    → z < h_ink: φ=1（油墨）
...
class DataGenerator:
    ...
    def target_phi_3d(
        ...
        if HAS_APERTURE:
            try:
                ...
                # Stage1 的 eta 被 eta_max=0.68 截断，但物理真实上限是 eta_max_aperture=0.85
                # 用 Stage1 的 theta + tanh 映射重新计算 eta
    ...
    def _center_opening_phi(self, x, y, z, eta, h_ink, r_open, interface_width):
        ...
        # 油墨边缘等效平均高度：体积守恒 h_ink/(1-eta)
        # 实际界面是弯月面，可高于 wall_height；接触线固定在围堰内壁边缘
        h_edge = h_ink / max(1.0 - eta, PHYSICS["ink_initial_fraction"])
    ...
    def _corner_blob_phi(self, x, y, z, eta, h_ink, interface_width):
        ...
        # 油墨边缘等效平均高度：体积守恒 h_ink/(1-eta)
        # 弯月面可高于 wall_height，接触线固定在围堰内壁边缘
        h_edge = h_ink / max(1.0 - eta, PHYSICS["ink_initial_fraction"])

        # blob 等效半径：1/4 圆柱近似，高度=wall_height
    ...
    def _sample_point_by_eta(self, eta: float) -> tuple:
        ...
        # z 方向：根据 η 动态调整范围，覆盖界面附近 + 全域
        # h_edge 是油墨边缘等效平均高度（体积守恒），可高于 wall_height
    ...
    def generate_all_data(self) -> dict[str, torch.Tensor]:
        ...
        for V_from, V_to, t in scenarios:
            ...
            # 计算目标 phi
            # 壁面 BC 点 (d_wall=0) 会落入 target_phi_3d 的毛细区域判断返回 NaN
            # 对于壁面 BC，直接根据位置赋 φ 值，不依赖 target_phi_3d 的毛细判断
            d_wall = min(x, self.Lx - x, y, self.Ly - y)
            ...
            if on_wall_top_z and d_wall < self.wall_top_z_tol:
                ...
                # 正常壁面区域：使用 target_phi_3d
                phi = self.target_phi_3d(x, y, z, t, V_to, V_prev=V_from)
                # 如果仍为 NaN（不应发生），根据 z 位置赋默认值
                if np.isnan(phi):
        ...
        t_jump = sample_continuous_times(n_dom_jump)
        ...
        for V_from, V_to, t in con_scenarios:
            ...
            # 不再使用独立的三区间 τ 模型（与 _analytical_contact_angle 不一致）
        ...
        # ============================================================
        # 原 contact_points/contact_theta 的精细采样逻辑保留，
        # 但目标值改用 target_phi_3d 计算（不依赖不可测的接触角 θ）
        ...
        for _ in range(n_cl):
            ...
            # 目标 φ 值用 target_phi_3d 计算（不依赖接触角 θ）
            phi_cl = self.target_phi_3d(x_cl, y_cl, z_cl, t, V, V_prev=V_prev)
            if not np.isnan(phi_cl):
                interface_points.append([x_cl, y_cl, z_cl, V_prev, V, t])
...
class Trainer:
    ...
    def _get_phase_mult(self, epoch: int) -> dict[str, float]:
        ...
        if epoch < s1_end:
            ...
            return {
                ...
                "ic_phi": 1.0,
                "eta_match": 0.0,  # 已禁用
                "phi_target3d": 0.0,  # 已禁用
                "phi_spatial": 1.0,  # 旧 geo 方案：spatial 乘子 = interface_mult
    ...
    def _compute_phi_spatial_loss(self, interface_mult: float):
        ...
        - spatial: 中心+边缘两点约束（中心 φ<0.3, 边缘 φ>0.7）
    ...
    def compute_eta_matching_loss(self, epoch: int) -> torch.Tensor:
        ...
        # 60 个 triplet 全覆盖：升压 6 档 × 5 时间 + 降压 6 档 × 5 时间
        # 每 epoch 随机选 12 个（梯度噪声=正则化），多 epoch 统计覆盖全空间
        import random

        all_triplets = []
        # 升压：6 档电压 × 5 个时间点
        for V in [5.0, 10.0, 15.0, 20.0, 25.0, 30.0]:
            for t in [0.005, 0.010, 0.020, 0.035, 0.050]:
        ...
        # 每 epoch 随机选 12 个 triplet
        selected = random.sample(all_triplets, min(12, len(all_triplets)))
        ...
        for V_from, V_to, t in selected:
            ...
            eta_pinn = self.compute_aperture_ratio_differentiable(V_from, V_to, t, n_grid=20)
