...
class EnhancedApertureModel(ApertureModel):
    ...
    def validate(self) -> dict[str, Any]:
        ...
        # 4. 瞬态单调性检查：η(V, t) 随 t 单调递增
        transient_mono = {"passed": True, "violations": []}
        test_voltages_t = [5, 10, 15, 20, 25, 30]
        time_points = [0.001, 0.005, 0.010, 0.020, 0.050]
        for V in test_voltages_t:
            prev_eta = -1.0
            for t in time_points:
                _, eta = self.theta_eta_from_triad(0.0, V, t)
                if eta < prev_eta - 1e-6:
                    transient_mono["passed"] = False
                    transient_mono["violations"].append((V, t, eta, prev_eta))
                prev_eta = eta

        if not transient_mono["passed"]:
            errors.append(f"瞬态单调性违规: {len(transient_mono['violations'])} 处")

        # 5. 电压响应单调性检查：η(V, t) 随 V 单调递增
        voltage_response = {"passed": True, "violations": []}
        test_voltages_v = [0, 5, 10, 15, 20, 25, 30]
        time_points_v = [0.005, 0.010, 0.020, 0.035, 0.050]
        for t in time_points_v:
            prev_eta = -1.0
            for V in test_voltages_v:
                _, eta = self.theta_eta_from_triad(0.0, V, t)
                if eta < prev_eta - 1e-6:
                    voltage_response["passed"] = False
                    voltage_response["violations"].append((V, t, eta, prev_eta))
                prev_eta = eta

        if not voltage_response["passed"]:
            errors.append(f"电压响应单调性违规: {len(voltage_response['violations'])} 处")

        # 6. 降压恢复趋势检查
        # 物理：降压后 η 应单调递减，最终趋近于 0
        # 注意：theta 是指数恢复，但 η=f(θ) 是非线性映射，
        # 所以 η 不严格遵循指数律，只检查趋势正确性
        recovery_check = {
            "passed": True,
            "violations": [],
            "monotonic": True,
        ...
        test_voltages_rec = [10, 20, 30]
        t_fall_points = [0.002, 0.005, 0.010, 0.020, 0.050]
        for V in test_voltages_rec:
            _, eta_0 = self.theta_eta_from_triad(0.0, V, 0.050)
            if eta_0 < 1e-6:
                continue
            prev_eta = eta_0
            for dt in t_fall_points:
                _, eta_actual = self.theta_eta_from_triad(V, 0.0, dt)
                # 趋势 1：η 应单调递减
                if eta_actual > prev_eta + 1e-6:
                    recovery_check["monotonic"] = False
                    recovery_check["passed"] = False
                    recovery_check["violations"].append((V, dt, "not_monotonic", eta_actual, prev_eta))
                prev_eta = eta_actual
            # 趋势 2：50ms 后 η 应接近 0（< 5% 初始值）
            _, eta_final = self.theta_eta_from_triad(V, 0.0, 0.050)
            if eta_final > 0.05 * eta_0 + 1e-6:
                recovery_check["converges_to_zero"] = False
                recovery_check["passed"] = False
                recovery_check["violations"].append((V, 0.050, "not_converged", eta_final, 0.05 * eta_0))

        if not recovery_check["passed"]:
            errors.append(
                f"降压恢复趋势违规: 单调性={recovery_check['monotonic']}, "
                f"收敛性={recovery_check['converges_to_zero']}, "
                f"{len(recovery_check['violations'])} 处"
            )

        # 7. 路径连续性趋势检查
        # 物理：升压和降压在相同 (V_to, t_since) 下 η 不同是正常的
        # （dynamic_response 和 surface_tension_recovery 时间常数不同）
        # 只检查：稳态时（t→∞）η 与路径无关，以及大电压下瞬态偏差收敛
        continuity_check = {
            "passed": True,
            "violations": [],
            "steady_state_match": True,
            "large_voltage_converge": True,
            "max_transient_diff": 0.0,
        }
        # 稳态检查：升压稳态 vs 降压稳态（V_from=V_to → 应该相同）
        test_voltages_ss = [5, 10, 15, 20, 25, 30]
        for V in test_voltages_ss:
            _, eta_rise = self.theta_eta_from_triad(0.0, V, 0.100)
            _, eta_fall = self.theta_eta_from_triad(V, V, 0.100)  # V_from=V_to → 稳态
            diff = abs(eta_rise - eta_fall)
            if diff > 0.01:
                continuity_check["steady_state_match"] = False
                continuity_check["passed"] = False
                continuity_check["violations"].append((V, "steady", eta_rise, eta_fall, diff))

        # 大电压瞬态检查：V_to=25,30V 时升压/降压偏差应 < 0.1
        test_voltages_lv = [25, 30]
        time_points_lv = [0.005, 0.010, 0.020, 0.035, 0.050]
        for V_to in test_voltages_lv:
            for t in time_points_lv:
                _, eta_rise = self.theta_eta_from_triad(0.0, V_to, t)
                _, eta_fall = self.theta_eta_from_triad(30.0, V_to, t)
                diff = abs(eta_rise - eta_fall)
                continuity_check["max_transient_diff"] = max(continuity_check["max_transient_diff"], diff)
                if diff > 0.10:
                    continuity_check["large_voltage_converge"] = False
                    continuity_check["passed"] = False
                    continuity_check["violations"].append((V_to, t, eta_rise, eta_fall, diff))

        if not continuity_check["passed"]:
            errors.append(
                f"路径连续性趋势违规: 稳态匹配={continuity_check['steady_state_match']}, "
                f"大电压收敛={continuity_check['large_voltage_converge']}, "
                f"最大瞬态偏差={continuity_check['max_transient_diff']:.4f}"
            )
        ...
        return {
            ...
            "steady_monotonicity": mono_results,
            "transient_monotonicity": transient_mono,
            "voltage_response": voltage_response,
            "eta_recovery": recovery_check,
