# constraints.py — 物理约束模块

> EWP INN 物理约束模块。包含 Navier-Stokes 残差计算、Laplace 压力残差、界面能量残差、Allen-Cahn 相场方程、电润湿驱动力、材料参数管理等。

> 文件路径: `/home/scnu/Gitee/EFD3D/src/physics/constraints.py`
> 总行数: 921
...
- **L107** `PhysicsConstraints`

### 公开函数

- **L137** `compute_navier_stokes_residual`
- **L398** `compute_laplace_pressure_residual`
- **L477** `compute_interface_energy_residual`
- **L514** `safe_compute_gradient`
- **L558** `get_grad`
- **L586** `compute_laplacian_and_hessian`
- **L617** `compute_core_residuals`
- **L881** `safe_compute_laplacian`

### 私有函数

- **L27** `_get_default_materials_params`
- **L46** `_get`
- **L110** `__init__`
- **L120** `_compute_capacitance`
- **L220** `_get_hessian_diag`
- **L359** `_compute_laplacian`
- **L541** `_compute_all_gradients`
- **L689** `_compute_temporal_smoothness`
- **L756** `_compute_vof_residual`
- **L901** `_empty_residual`
...
```python
   ...
   6 | import logging
     ...
     |
  11 | logger = logging.getLogger("EWPINN_Physics")
     |
     | # 导入统一物理配置
     | try:
  ...
  21 |     PHYSICS = {}
     ...
     |
  26 |
  27 | def _get_default_materials_params() -> dict:
     |     """获取默认材料参数（优先从统一配置加载）"""
     |     global _DEFAULT_MATERIALS
     |     if _DEFAULT_MATERIALS is not None:
  31 |         return _DEFAULT_MATERIALS.copy()
     |
     |     try:
     |         from src.config import get_materials_params
     |
  36 |         _DEFAULT_MATERIALS = get_materials_params()
     |         return _DEFAULT_MATERIALS.copy()
     |     except Exception:
     |         pass
     |
  41 |     # 回退到 PHYSICS 配置（单一来源）
     |     # 所有物理参数必须从 PHYSICS 字典导入，禁止本地硬编码（R1 规则）
     |     # 如果 PHYSICS 不可用，抛出异常而不是使用错误的默认值
     |     from src.config import PHYSICS
     |
  46 |     def _get(key, required=True):
     |         """从 PHYSICS 获取参数，缺失时抛出异常"""
     |         if key in PHYSICS:
     |             return PHYSICS[key]
     |         if required:
  51 |             msg = f"PHYSICS['{key}'] missing, cannot compute physics constraints"
     |             raise KeyError(msg)
     |         return None
     |
     |     return {
  ...
  96 |         # Allen-Cahn 相场参数
     ...
     |         # 阈值电压
 101 |         "V_T_base": _get("V_T_base"),
     |         # 侧壁 Teflon 污染接触角
     |         "theta_wall_teflon": _get("theta_wall_teflon"),
     |     }
     |
 106 |
 107 | class PhysicsConstraints:
     |     """物理约束类 - 处理Navier-Stokes方程和材料属性"""
     |
 110 |     def __init__(self, materials_params=None):
 111 |         # 从统一配置获取默认参数，允许外部覆盖
 ...
 116 |
     ...
     |         self.global_step = 0
     |
 120 |     def _compute_capacitance(self, with_oil=False):
 121 |         eps0 = self.materials_params.get("epsilon_0", 8.854e-12)
 ...
 126 |         A_eff = self.materials_params.get("A_eff", 1.20)
     |
     |         Z = d_su8 / (eps0 * eps_su8) + d_teflon / (eps0 * eps_teflon)
     |
     |         if with_oil:
 131 |             eps_oil = self.materials_params.get("epsilon_ink", 4.0)
     |             h_ink = self.materials_params.get("ink_thickness", 3e-6)
     |             Z = Z + h_ink / (eps0 * eps_oil)
     |
     |         return A_eff / Z
 136 |
 137 |     def compute_navier_stokes_residual(self, x, predictions, model=None, grads=None):
     |         """
 ...
 146 |         """
     ...
     |
 151 |             if not isinstance(x, torch.Tensor):
 ...
 171 |             rho_polar = self.materials_params["density_polar"]
 ...
 196 |                 lap_u, lap_v, lap_w = grads["lap_u"], grads["lap_v"], grads["lap_w"]
 ...
 216 |                 lap_v = self._compute_laplacian(v, x)
     |                 lap_w = self._compute_laplacian(w, x)
     |                 # 回退路径：独立计算 phi 的 Hessian 各分量（用于曲率）
     |                 # 注意：不能用 Laplacian 代替各二阶导数，曲率公式需要独立分量
 220 |                 def _get_hessian_diag(first_grad):
 221 |                     """从一阶梯度计算 Hessian 对角线和混合分量"""
     |                     hess = {}
 ...
 231 |                         if g2 is not None:
     ...
     |                         else:
     |                             for j, key_j in enumerate("xyz"):
 236 |                                 hess[key_i + key_j] = torch.zeros_like(phi)
     |                     return hess
     |
     |                 g_phi = torch.autograd.grad(
     |                     outputs=phi.sum(), inputs=x,
 241 |                     create_graph=True, retain_graph=True, allow_unused=True,
     |                 )[0]
     |                 if g_phi is not None:
     |                     phi_hess = _get_hessian_diag(g_phi)
     |                     phi_xx = phi_hess["xx"]
 246 |                     phi_yy = phi_hess["yy"]
     |                     phi_zz = phi_hess["zz"]
     |                     phi_xy = phi_hess["xy"]
     |                     phi_xz = phi_hess["xz"]
     |                     phi_yz = phi_hess["yz"]
 ...
 321 |             use_convection = self.materials_params.get("use_convection", False)
     |             if use_convection:
 ...
 346 |             momentum_w = rho * (w_t + w_conv) + p_z - viscous_w - f_st_z - f_ew_z
     |
     |             return {
     |                 "continuity": continuity,
     |                 "momentum_u": momentum_u,
 351 |                 "momentum_v": momentum_v,
     |                 "momentum_w": momentum_w,
     |             }
     |
     |         except Exception as e:
 356 |             logger.error(f"Navier-Stokes残差计算失败: {e!s}")
     |             return self._empty_residual(x, predictions)
     |
 359 |     def _compute_laplacian(self, scalar_field, coords, spatial_dims=3):
     |         """计算标量场的拉普拉斯算子"""
 361 |         try:
 ...
 366 |             first_grad = torch.autograd.grad(
     |                 outputs=scalar_field,
     |                 inputs=coords,
     |                 grad_outputs=grad_outputs,
     |                 create_graph=True,
 371 |                 retain_graph=True,
     |                 allow_unused=True,
     |             )[0]
     |
     |             if first_grad is None:
 376 |                 return laplacian
     |
     |             # 计算二阶梯度 (对角线元素之和)
     |             for i in range(min(spatial_dims, first_grad.shape[-1])):
     |                 grad_i = first_grad[:, i]
 381 |                 second_grad = torch.autograd.grad(
     |                     outputs=grad_i,
     |                     inputs=coords,
     |                     grad_outputs=torch.ones_like(grad_i),
     |                     create_graph=True,
 386 |                     retain_graph=True,
     |                     allow_unused=True,
     |                 )[0]
     |
     |                 if second_grad is not None and i < second_grad.shape[-1]:
 391 |                     laplacian = laplacian + second_grad[:, i]
     |
     |             return laplacian
     |
     |         except Exception:
 396 |             return torch.zeros_like(scalar_field)
     |
 398 |     def compute_laplace_pressure_residual(self, x_phys: torch.Tensor, predictions: torch.Tensor, grads=None):
     |         """
 ...
 401 |
     ...
     |
 406 |         计算: κ_xy = -(φ_xx + φ_yy)/|∇φ|  (xy 平面曲率分量)
     |               沿界面采样点计算 κ_xy 的方差作为 loss
     |
     |         Args:
     |             x_phys: (batch, 6) 物理点坐标
 411 |             predictions: (batch, 5) 模型预测
     |         """
     |         try:
 ...
 441 |                 # 回退：独立计算
     ...
     |                 except Exception:
     |                     return res
 446 |                 if grad_phi is None:
     |                     return res
     |                 phi_x = grad_phi[:, 0]
     |                 phi_y = grad_phi[:, 1]
     |                 phi_z = grad_phi[:, 2]
 451 |                 _grad_mag_sq, grad_mag = gradient_magnitude(phi_x, phi_y, phi_z)
     |                 try:
 ...
 476 |
 477 |     def compute_interface_energy_residual(self, x_phys, predictions, grads=None):
     |         """界面能 — 纯 sigma*|grad(phi)|, 塑造圆润液滴
     |
     |         注意：电润湿力已作为体积力加入 NS 方程（f_ew_x, f_ew_y）。
 481 |         AC 方程中不再有独立的 EW 源项。
     |         此项只做表面张力最小化(最小界面面积 = 圆润形状)。
     |         """
     |         try:
     |             device = x_phys.device if isinstance(x_phys, torch.Tensor) else torch.device("cpu")
 486 |             zero = torch.zeros(x_phys.shape[0], device=device, requires_grad=True)
     |             res = {"interface_energy": zero}
     |             if not (isinstance(x_phys, torch.Tensor) and predictions.dim() >= 2):
     |                 return res
     |             phi = predictions[:, 4]
 491 |             # 从统一梯度计算结果读取 phi 梯度
     ...
     |             if grads is not None:
 496 |                 grad_mag = torch.sqrt(grads["phi_x"] ** 2 + grads["phi_y"] ** 2 + grads["phi_z"] ** 2 + 1e-10)
     |             else:
     |                 try:
 ...
 501 |                     return res
     ...
     |
 506 |             sigma = self.materials_params.get("surface_tension_polar_ink", 0.02505)
     |             res["interface_energy"] = sigma * grad_mag
     |             return res
     |         except Exception as e:
     |             logger.warning(f"界面能失败: {e}")
 511 |             device = x_phys.device if isinstance(x_phys, torch.Tensor) else torch.device("cpu")
     |             return {"interface_energy": torch.zeros(x_phys.shape[0], device=device, requires_grad=True)}
     |
 514 |     def safe_compute_gradient(self, output: torch.Tensor, input_tensor: torch.Tensor):
     |         try:
 516 |             if not isinstance(output, torch.Tensor) or not isinstance(input_tensor, torch.Tensor):
     |                 return torch.zeros_like(input_tensor)
     |             if not input_tensor.requires_grad:
     |                 input_tensor = input_tensor.clone().requires_grad_(True)
     |             grad_outputs = torch.ones_like(output)
 521 |             grad = torch.autograd.grad(
     |                 outputs=output,
     |                 inputs=input_tensor,
     |                 grad_outputs=grad_outputs,
     |                 create_graph=True,
 526 |                 retain_graph=True,
     |                 allow_unused=True,
     |             )[0]
     |             if grad is None:
     |                 grad = torch.zeros(
 531 |                     (*input_tensor.shape[:-1], input_tensor.shape[-1]),
     |                     device=input_tensor.device,
     |                 )
     |             return grad
     |         except Exception:
 536 |             return torch.zeros(
     |                 (*input_tensor.shape[:-1], input_tensor.shape[-1]),
     |                 device=input_tensor.device,
     |             )
     |
 541 |     def _compute_all_gradients(self, x_phys: torch.Tensor, predictions: torch.Tensor) -> dict[str, torch.Tensor]:
     |         """
     |         统一计算所有一阶和二阶梯度。
     |
     |         一次性计算 u,v,w,p,phi 的所有空间梯度和时间导数，
 546 |         以及 Laplacian 和二阶混合导数（用于曲率）。
     |
     |         Returns:
     |             grads dict，键名格式：
     |             一阶: u_x, u_y, u_z, u_t, v_x, ..., phi_x, phi_y, phi_z, phi_t
 551 |             二阶: lap_u, lap_v, lap_w, lap_phi
 ...
 601 |         grads["lap_u"] = lap_u
     |         grads["lap_v"] = lap_v
     |         grads["lap_w"] = lap_w
     |
     |         # phi 需要完整 Hessian（用于曲率计算）
 606 |         lap_phi, phi_hess = compute_laplacian_and_hessian(g_phi, compute_hessian=True)
     |         grads["lap_phi"] = lap_phi
     |         grads["phi_xx"] = phi_hess["xx"]
     |         grads["phi_yy"] = phi_hess["yy"]
     |         grads["phi_zz"] = phi_hess["zz"]
 611 |         grads["phi_xy"] = phi_hess["xy"]
     |         grads["phi_xz"] = phi_hess["xz"]
     |         grads["phi_yz"] = phi_hess["yz"]
     |
     |         return grads
 616 |
 617 |     def compute_core_residuals(
     |         self,
     |         x_phys: torch.Tensor,
     |         predictions: torch.Tensor,
 621 |         model: nn.Module | None = None,
     |     ) -> dict[str, torch.Tensor]:
     |         """
     |         统一物理残差计算入口 - 作为"唯一物理真相"
     |
 626 |         先统一计算所有梯度，再分发给各约束子方法。
 ...
 661 |         except Exception as e:
     ...
     |         # 4. Laplace 压力一致约束
     |         try:
 666 |             lp_residuals = self.compute_laplace_pressure_residual(x_phys, predictions, grads=grads)
     |             residuals.update(lp_residuals)
     |         except Exception as e:
     |             logger.warning(f"Laplace 压力残差计算失败: {e}")
     |
 671 |         # 5. 时间连续性正则化（需要 model 做三时间点前向）
     |         try:
     ...
     |         except Exception as e:
 676 |             logger.warning(f"时间连续性残差计算失败: {e}")
     |             residuals["temporal_smoothness"] = torch.zeros(batch_size, device=device)
     |
     |         # 7. 压力钉扎
     |         try:
 681 |             p = predictions[:, 3] if predictions.shape[1] >= 4 else None
     |             if p is not None:
 ...
 686 |
     |         return residuals
     |
 689 |     def _compute_temporal_smoothness(
     |         self,
 691 |         x_phys: torch.Tensor,
     |         predictions: torch.Tensor,
 ...
 711 |
     ...
     |             x_next[:, 5] = t_since + dt
     |             x_prev = x_phys.clone()
 716 |             x_prev[:, 5] = torch.clamp(t_since - dt, min=0.0)
     |
     |             # 拼接为 (2*batch, 6)，一次前向
 ...
 736 |
     ...
     |             v_prev = pred_prev[:, 1]
     |             w_prev = pred_prev[:, 2]
 741 |             phi_prev = pred_prev[:, 4] if pred_prev.shape[1] >= 5 else torch.zeros_like(u_prev)
     |
     |             # 二阶差分 (加速度/曲率)
     |             acc_u = (u_next - 2 * u_t + u_prev) ** 2
     |             acc_v = (v_next - 2 * v_t + v_prev) ** 2
 746 |             acc_w = (w_next - 2 * w_t + w_prev) ** 2
 ...
 751 |
     ...
     |             return torch.zeros(batch_size, device=device)
     |
 756 |     def _compute_vof_residual(
     |         self,
     |         x_phys: torch.Tensor,
     |         predictions: torch.Tensor,
     |         model: nn.Module | None = None,
 761 |         grads: dict[str, torch.Tensor] | None = None,
     |     ) -> torch.Tensor:
     |         """
     |         计算 Allen-Cahn 相场方程残差 (标准化相场模型)
     |
 766 |         标准形式: ∂φ/∂t + u·∇φ = M [ σ·ε·∇²φ − (σ/ε)·W'(φ) ]
     |
     |         - ε: 界面宽度 (m), 默认 5μm
     |         - σ: 界面张力 (N/m), 使用 surface_tension_polar_ink
     |         - M: 迁移率 (m³·s/kg), 控制界面移动速度
 771 |         - W'(φ) = 2φ(1-φ)(1-2φ): 双阱势导数
     |
 ...
 776 |         Args:
     ...
     |         """
     |         device = x_phys.device if isinstance(x_phys, torch.Tensor) else torch.device("cpu")
 781 |         batch_size = x_phys.shape[0] if isinstance(x_phys, torch.Tensor) else 1
     |
     |         try:
 ...
 786 |
     ...
     |
     |             if model is not None:
 791 |                 predictions = model(x_phys)
     |
     |             u, v, w = predictions[:, 0], predictions[:, 1], predictions[:, 2]
     |             phi = predictions[:, 4] if predictions.shape[1] >= 5 else torch.zeros_like(u)
     |
 796 |             # 从统一梯度计算结果读取（由 compute_core_residuals 提供）
     |             if grads is not None:
     |                 phi_x = grads["phi_x"]
     |                 phi_y = grads["phi_y"]
     |                 phi_z = grads["phi_z"]
 801 |                 phi_t = grads["phi_t"]
     |                 lap_phi = grads["lap_phi"]
     |             else:
     |                 # 回退：独立计算
     |                 g_phi = torch.autograd.grad(
 806 |                     phi.sum(),
     ...
     |                     create_graph=True,
     |                     retain_graph=True,
 811 |                     allow_unused=True,
     |                 )[0]
     |                 if g_phi is None:
     |                     return torch.zeros(batch_size, device=device)
     |                 phi_x, phi_y, phi_z = g_phi[:, 0], g_phi[:, 1], g_phi[:, 2]
 816 |                 phi_t = g_phi[:, 5] if x_phys.shape[1] >= 6 else torch.zeros_like(phi)
 ...
 861 |             # 量纲: [m³·s/kg]·[F/m²]·[V²]/[m] = [m³·s/kg]·[J/m²]/[m] = [m³·s/kg]·[N/m²] = [m²/s]
     |             # 需要再除以 eps_ac → [m²/s]/[m] = [m/s]
     |             # 不对，让我重新算：
     |             # M_ac [m³·s/kg] * delta_C [F/m²] * V² [V²] / eps_ac [m]
     |             # = [m³·s/kg] * [C/V·m²] * [V²] / [m]
 866 |             # = [m³·s/kg] * [A·s/V·m²] * [V²] / [m]
     |             # = [m³·s/kg] * [A·s·V / m³]
     |             # = [m³·s/kg] * [J / m³]  (因为 J = A·s·V)
     |             # = [m³·s/kg] · [kg·m/s² · m / m³]
     |             # = [m²/s]
 871 |             # 注意: 电润湿力已作为体积力加入 NS 方程（f_ew_x, f_ew_y）
     |             # AC 方程中不再需要独立的 EW 源项
 ...
 876 |
     ...
     |             return torch.zeros(batch_size, device=device)
     |
 881 |     def safe_compute_laplacian(self, scalar_field: torch.Tensor, coords: torch.Tensor):
     |         try:
 ...
 896 |                     lap = lap + g2[..., i]
     ...
     |
 901 |     def _empty_residual(self, x, predictions):
     |         """返回空的残差字典"""
     |         device = torch.device("cpu")
     |         if isinstance(x, torch.Tensor):
     |             device = x.device
 906 |         elif isinstance(predictions, torch.Tensor):
     |             device = predictions.device
     |
     |         batch_size = 1
     |         if isinstance(x, torch.Tensor):
 911 |             batch_size = x.shape[0]
     |         elif isinstance(predictions, torch.Tensor):
     |             batch_size = predictions.shape[0]
     |
     |         return {
 916 |             "continuity": torch.zeros(batch_size, device=device),
